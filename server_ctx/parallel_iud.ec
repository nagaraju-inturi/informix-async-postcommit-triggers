...
//    sprintf(demoquery1, "insert into usertable (YCSB_KEY, FIELD1, FIELD2, FIELD3, FIELD4) values (0, ?::JSON, ?, ?, ?)");
    sprintf(demoquery1, "insert into scores (playerid, score) values (?, ?)");
    ...
    for (i=0; i < row_count; i ++)
        ...
        EXEC SQL PUT :cursor_name FROM :field1_val, :field2_val;
...
*/
    ...
    sprintf(demoquery1, "update scores set score=? where playerid=?");
    $set isolation to dirty read;
    $select max(playerid) into :maxval from scores;
    $select min(playerid) into :minval from scores;
    ...
    for (i=0; i < row_count; i ++)
        ...
        $execute demostmt2 using :field1_val,:cli_id_cli;
        if (sqlca.sqlcode != 0)
...
do_select(int row_count)
{
   ...
   field4_val[0] = '\0';
    ...
    sprintf(demoquery1, "select score from scores where playerid=?");
    $set isolation to dirty read;
    /* $select max(YCSB_KEY) into :maxval from usertable; */
    $select max(m) into :maxval from (select max(playerid) m from scores);
    /* $select min(YCSB_KEY) into :minval from usertable; */
    $select min(m) into :minval from (select min(playerid) m from scores);
    $set isolation to committed read;
...
*/
    ...
    for (i=0; i < row_count; i ++)
        ...
        $execute demostmt3 into :field1_val using :cli_id_cli;
        if (sqlca.sqlcode != 0)
...
do_select2(int row_count)
{
    ...
    $char cursor_name[100]; 
    $char demostmt1[100]; 
    $static char  stmt[] = "select score from scores where playerid between ? and ?";
    int i,j;
...
*/
    ...
    for (i=0; i < row_count; i ++)
        ...
        for(;;)
           {
           $fetch :cursor_name into :field1_val;
...
int *row_count;
{
    ...
    if (strcasecmp(isTrusted, "T") == 0)
       {
       $connect to "gamedb" as :conn;
       }
    else
       {
       $connect to "gamedb" as :conn user :userID using :password;
       }
    if (SQLCODE)
